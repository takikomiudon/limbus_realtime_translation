...
Cloud Build Trigger が repository root と `server/` のどちらを source root にしても同じ設定を読めるように、Cloud Buildpacks の Python runtime と entrypoint は両方の `project.toml` で固定しています。
